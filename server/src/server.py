# -*- coding: utf-8 -*-
"""
Agora Agent & Token Service — Interruptions Recipe

HTTP APIs:
- GET  /get_config     -> Generate connection config
- POST /startAgent     -> Start agent with Interruptions config
...
# FastAPI application
app = FastAPI(
    title="Agora Interruptions Recipe Service",
    version="1.0.0",
    description="Agora Conversational AI with configurable interruption behavior",
)
...
def _generate_channel_name() -> str:
    return f"interruptions-{int(time.time())}-{random.randint(1000, 9999)}"
...
async def start_agent(request: StartAgentRequest):
    """Start agent with Interruptions config in a channel"""
    if agent is None:
        raise HTTPException(
            status_code=500,
            detail="Service not properly configured. Please check environment variables.",
